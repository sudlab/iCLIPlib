    ...
    def __call__(self,
        ...
        if sort:
            statement = ("/bin/bash -c "
                         "'python %(working_dir)s/../scripts/%(options)s %(stdin)s |sort > %(stdout)s'") % locals()
        else:
            statement = ("/bin/bash -c "
                         "'python %(working_dir)s/../scripts/%(options)s %(stdin)s > %(stdout)s'") % locals()
...
def test_tool():
    ...
    tool_dir = os.path.join(os.path.dirname(testing_dir), "scripts")
